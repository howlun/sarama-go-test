...
// to store offsets.
func NewZookeeperOffsetManager(cg *GroupConsumerService, config *OffsetManagerConfig) OffsetManager {
	if config == nil {
...
func (zom *zookeeperOffsetManager) FinalizePartition(topic string, partition int32, lastOffset int64, timeout time.Duration) error {
	...
	if lastOffset >= 0 {
		if lastOffset-tracker.highestProcessedOffset > 0 {
			log.Printf("%s/%d :: Last processed offset: %d. Waiting up to %ds for another %d messages to process...", topic, partition, tracker.highestProcessedOffset, timeout/time.Second, lastOffset-tracker.highestProcessedOffset)
...
func (zom *zookeeperOffsetManager) commitOffset(topic string, partition int32, tracker *partitionOffsetTracker) error {
	...
	if err != nil {
		log.Printf("FAILED to commit offset %d for %s/%d!", tracker.highestProcessedOffset, topic, partition)
	} else if zom.config.VerboseLogging {
		log.Printf("Committed offset %d for %s/%d!", tracker.lastCommittedOffset, topic, partition)
	}
